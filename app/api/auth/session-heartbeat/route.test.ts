...
const { mockProxyToBackend } = vi.hoisted(() => ({ mockProxyToBackend: vi.fn() }));
...
describe('session-heartbeat route', () => {
  it('proxies POST to the backend session-heartbeat endpoint', async () => {
    mockProxyToBackend.mockResolvedValue(
      new Response(JSON.stringify({ days_since_first_session: 3, previous_sessions: 5 }), {
        status: 200,
      })
    );
    ...
    expect(mockProxyToBackend).toHaveBeenCalledWith(req, '/auth/session-heartbeat');
